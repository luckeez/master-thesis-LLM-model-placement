...
from layout_synthesizer import LayoutMethod, LayoutSynthesizer
...
def visualize_final_cluster():
    ...
    machine_num_dict = {"A100": 4, "T4": 8, "L4": 8}
...
def visualize_initial_cluster(config_file_path: str, show_edges: bool = True):
    ...
    for u, v in graph.edges():
        ...
        if gu == gv and gu != -1:
            ...
            graph.edges[u, v]["width"] = 2
        else:
            graph.edges[u, v]["color"] = "black"
            graph.edges[u, v]["width"] = 1
    
    init_pos = {}
    for i, (g, nodes) in enumerate(groups.items()):
        if len(nodes) > 1 and g != -1:
            base = np.array([i, 0])
            for j, n in enumerate(nodes):
                r = j//2
                c = j%2
                init_pos[n] = base + np.array([r*0.3, -c*0.3])
    
    if len(init_pos) == 0:
        pos = nx.spring_layout(graph, seed=0)
    else:
        pos = nx.spring_layout(graph, pos=init_pos, fixed=init_pos.keys(), seed=0)  # positions for all nodes
    ...
    nx.draw_networkx_labels(graph, pos, labels, font_size=10, font_family="sans-serif", font_color="white")
...
def main():
    visualize_initial_cluster(config_file_path="./config/single12.ini", show_edges=True)
