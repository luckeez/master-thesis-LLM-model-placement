...
def visualize_initial_cluster(config_file_path: str, show_edges: bool = True):
    ...
    labels = {n: re.search(r"(\d+(?:_\d+)*)", n).group(1) for n in graph.nodes() if n not in ["Source", "Sink"]}
...
def main():
    visualize_initial_cluster(config_file_path="./config/single12mach_socket.ini", show_edges=True)
