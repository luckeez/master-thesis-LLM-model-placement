...
def visualize_initial_cluster(config_file_path: str, show_edges: bool = True):
    ...
    for section in config.sections():
        if not section.startswith("MachineNode"):
            continue
        node_type="Machine"
        group = int(config[section]["group"])
        graph.add_node(section, type=node_type, group=group)
        color_map.append("gray")

        connected_nodes = eval(config[section]["connected_nodes"])
        for c in connected_nodes:
            if c == "source":
                graph.add_edge("Source", section, color="black", width=1)
            elif c == "sink":
                graph.add_edge(section, "Sink", color="black", width=1)
            else:
                c = f"MachineNode-{c}"
                graph.add_edge(section, c, color="black", width=1)
    ...
    for section in config.sections():
        if not section.startswith("SocketNode"):
            continue
        node_type = config[section]["type"]
        group = int(config[section]["group"])
        graph.add_node(section, type=node_type, group=group)
    ...
    for section in config.sections():
        if not section.startswith("PciNode"):
            continue
        node_type = config[section]["type"]
        group = int(config[section]["group"])
        graph.add_node(section, type=node_type, group=group)
    ...
    for section in config.sections():
        if not section.startswith("InternalLink"):
            continue
        node1 = config[section]["from_node"]
        node2 = config[section]["to_node"]

        if config[section]["type"] == "Internal":
            graph.add_edge(node1, node2, color="black", width=1)
        elif config[section]["type"] == "PCIe":
            graph.add_edge(node1, node2, color="blue", width=1)
        elif config[section]["type"] == "NVLink":
            graph.add_edge(node1, node2, color="green", width=2)
    ...
    labels = {n: n[0] + re.search(r"(\d+(?:_\d+)*)", n).group(1) for n in graph.nodes() if n not in ["Source", "Sink"]}
...
def main():
    visualize_initial_cluster(config_file_path="./config/double12mach_socket.ini", show_edges=True)
