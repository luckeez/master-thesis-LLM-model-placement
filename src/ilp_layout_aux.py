...
class ILPLayout:
    ...
    def build_model(self, seed: int, model_name: str) -> Tuple[int, int, int, int]:
        ...
        if not self.enable_memory:
            ...
            final_objective = self.opt_alpha * norm_throughput - (1 - self.opt_alpha) * norm_memory_cost
    ...
    def search_layout(self, max_run_time: float, early_stop_threshold: float, early_stop_time: float,
        ...
        if not self.enable_memory:
            ...
            self.opt_upper_bound = self.opt_alpha * 1.0 - (1 - self.opt_alpha) * (min_model_memory / self.total_vram)  # best possible memory cost
